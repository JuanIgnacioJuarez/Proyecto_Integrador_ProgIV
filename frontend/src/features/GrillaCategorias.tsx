...
export function GrillaCategorias({ onEditar, action }: GrillaCategoriasProps) {
  ...
  const [productoPopup, setProductoPopup] = useState<ProductoPopup | null>(null);
  ...
  const allRows = useMemo<CategoriaRow[]>(() => {
    ...
    for (const root of roots) {
      ...
      if (hijos.length === 0) {
        pushRow(root, [Number(root.id)], [root.nombre]);
        continue;
      }
      for (const hijo of hijos) {
        if (!hijo.id) continue;
  ...
  const rowsFiltradas = useMemo(() => {
    ...
    if (term) {
      rows = rows.filter((row) =>
        `${row.categoria} ${row.subcategoria} ${row.subcategoria2}`.toLowerCase().includes(term),
      );
    }
  ...
  const categoryGroupSizeByStart = useMemo(() => {
    ...
    while (i < rowsPagina.length) {
      let j = i + 1;
      while (j < rowsPagina.length && rowsPagina[j].categoria === rowsPagina[i].categoria) {
        j += 1;
      }
      groupSizeByStart.set(i, j - i);
      i = j;
    }
  ...
  const subcategoryGroupSizeByStart = useMemo(() => {
    ...
    while (i < rowsPagina.length) {
      ...
      while (
        j < rowsPagina.length &&
        rowsPagina[j].categoria === rowsPagina[i].categoria &&
        rowsPagina[j].subcategoria === rowsPagina[i].subcategoria
      ) {
        j += 1;
      }
      groupSizeByStart.set(i, j - i);
  ...
  const handleSortChange = (
    nextSortBy: "" | "categoria" | "subcategoria" | "subcategoria2",
    value: string,
  ) => {
    if (!value) {
  ...
  useEffect(() => {
    ...
    if (state.restoreState) {
      ...
      if (
        nextSortBy === "" ||
        nextSortBy === "categoria" ||
        nextSortBy === "subcategoria" ||
        nextSortBy === "subcategoria2"
      ) {
        setSortBy(nextSortBy);
        consumed = true;
      }
      if (nextSortDir === "" || nextSortDir === "asc" || nextSortDir === "desc") {
    ...
    if (
      typeof state.highlightCategoryId === "number" &&
      Number.isFinite(state.highlightCategoryId) &&
      state.highlightCategoryId > 0
    ) {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => setHighlightedCategoryId(null), 2500);
  ...
  const getCategoriaById = (categoriaId?: number) => {
    if (!categoriaId) return undefined;
    return categoriasPorId.get(categoriaId);
  };
  ...
  const renderNivel = (
    ...
    label: string,
    info: { total: number; items: string[] },
    ...
    return (
      <div className={`relative inline-flex items-center gap-2 group ${align === "center" ? "justify-center" : ""}`}>
        ...
        <button
          type="button"
          onClick={() => openProductosPopup(categoriaId)}
          className={`text-left text-sm text-blue-700 hover:underline ${
            isHighlighted(categoriaId) ? "bg-amber-100 border border-amber-300 rounded px-1 py-0.5 animate-pulse" : ""
          }`}
        >
          {label}
        </button>
        <div className="absolute left-0 top-full z-20 mt-0 hidden w-64 rounded-md border border-gray-200 bg-white p-2 text-xs text-gray-700 shadow-lg group-hover:block group-focus-within:block">
          <p>
            Total productos: <span className="font-semibold text-gray-900">{info.total}</span>
          </p>
          <button
            type="button"
            onClick={() => openProductosPopup(categoriaId)}
            className="text-blue-700 mt-1 underline underline-offset-2 hover:text-blue-900"
          >
  ...
  const selectedCategorias = Array.from(selectedIds)
    .map((id) => categoriasPorId.get(id))
    .filter((cat): cat is Categoria => Boolean(cat));
      ...
      {canManageCatalogo && (
        <div className="mb-4 rounded-xl border border-gray-200 bg-white px-4 py-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <InfoHint
              text="Apreta Seleccionar, marca las categorias o subcategorias que quieras y despues toca Editar o Desactivar."
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={() => {
                ...
                const categoria = categoriasPorId.get(onlyId);
                if (categoria) {
                  onEditar(categoria, {
                    returnPage: currentPageSafe,
            ...
            <button
              type="button"
              onClick={() => {
                if (!selectedActivas.length) return;
                if (
                  window.confirm(
                    `Desactivar ${selectedActivas.length} categoria(s) seleccionada(s)?`,
                  )
                ) {
                  for (const cat of selectedActivas) {
                    if (cat.id) cambiarEstado(cat.id, false);
                  }
            ...
            <button
              ...
              onClick={() => {
                ...
                if (
                  window.confirm(
                    `Reactivar ${selectedInactivas.length} categoria(s) seleccionada(s)?`,
                  )
                ) {
                  for (const cat of selectedInactivas) {
                    if (cat.id) cambiarEstado(cat.id, true);
                  }
                  setSelectedIds(new Set());
                }
              }}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const ok1 = window.confirm(
                    `Eliminar definitivamente ${selectedInactivas.length} categoria(s) inactiva(s)? Esta accion no se puede deshacer.`,
                  );
                  if (!ok1) return;
                  const ok2 = window.confirm(
                    "Confirmacion final: verificaste que no tengan hijas ni productos asociados?",
                  );
                  if (!ok2) return;
                  for (const cat of selectedInactivas) {
                    if (cat.id) eliminarDefinitivo(cat.id);
                  }
                  setSelectedIds(new Set());
                }}
                disabled={!selectionMode || selectedInactivas.length === 0 || selectedIds.size === 0}
            ...
            <table className="min-w-full divide-y divide-gray-200/70">
                ...
                <tr>
                  <th scope="col" className="px-6 py-3 text-left border-r border-gray-100">
                    <div className="flex flex-col gap-1">
                      ...
                      <select
                        value={sortBy === "categoria" ? sortDir : ""}
                        onChange={(e) => handleSortChange("categoria", e.target.value)}
                  ...
                  <th scope="col" className="px-6 py-3 text-left border-r border-gray-100">
                    <div className="flex flex-col gap-1">
                      ...
                      <select
                        value={sortBy === "subcategoria" ? sortDir : ""}
                        onChange={(e) => handleSortChange("subcategoria", e.target.value)}
                        className="text-[11px] text-gray-600 border border-gray-200 rounded px-1.5 py-0.5 bg-white"
                      >
                        <option value="">Orden</option>
                        <option value="asc">A-Z</option>
                  ...
                  <th scope="col" className="px-6 py-3 text-left border-r border-gray-100">
                    <div className="flex flex-col gap-1">
                      ...
                      <select
                        value={sortBy === "subcategoria2" ? sortDir : ""}
                        onChange={(e) => handleSortChange("subcategoria2", e.target.value)}
                        className="text-[11px] text-gray-600 border border-gray-200 rounded px-1.5 py-0.5 bg-white"
                      >
                        <option value="">Orden</option>
                        <option value="asc">A-Z</option>
              ...
              <tbody className="bg-white divide-y divide-gray-200/70">
                {rowsPagina.map((row, idx) => {
                  ...
                  const subcategoriaRepetida =
                    idx > 0 &&
                    rowsPagina[idx - 1].categoria === row.categoria &&
                    rowsPagina[idx - 1].subcategoria === row.subcategoria;
                  ...
                  const subInfo = row.subcategoriaId
                    ? productosPorCategoria.get(row.subcategoriaId) || { total: 0, items: [] }
                    : { total: 0, items: [] };
                  const sub2Info = row.subcategoria2Id
                    ? productosPorCategoria.get(row.subcategoria2Id) || { total: 0, items: [] }
                    : { total: 0, items: [] };
                  ...
                  >
                    {!categoriaRepetida && (
                      <td
                        rowSpan={categoryGroupSizeByStart.get(idx) || 1}
                        className={`px-6 py-4 align-middle text-center border-r border-gray-100 ${
                          categoriaInactiva ? "bg-gray-300 text-gray-700" : ""
                        }`}
                      >
                    ...
                    {!subcategoriaRepetida && (
                      <td
                        rowSpan={subcategoryGroupSizeByStart.get(idx) || 1}
                        className={`px-6 py-4 align-middle border-r border-gray-100 ${
                          subcategoriaInactiva ? "bg-gray-300 text-gray-700" : ""
                        }`}
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-xl border border-gray-200 bg-white p-5 shadow-xl">
            <div className="flex items-start justify-between gap-3 mb-4">
              <h3 className="text-lg font-bold text-gray-900">{productoPopup.categoriaNombre}</h3>
              <button
                type="button"
                onClick={() => {
                  setProductoPopup(null);
            ...
            <p className="text-sm text-gray-700 mb-3">
              ...
              <span className="font-semibold text-gray-900">{productoPopup.total}</span>
            </p>

            <div className="max-h-64 overflow-y-auto pr-1 space-y-2">
              {productoPopup.items.length === 0 ? (
                <p className="text-sm text-gray-500">No hay productos asociados.</p>
              ) : (
                productoPopup.items.map((nombre) => (
